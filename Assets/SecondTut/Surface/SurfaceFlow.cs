...
public class SurfaceFlow : MonoBehaviour {
	...
	private void PositionParticles () {
		...
		NoiseMethod method = NoiseHash.methods[(int)surface.type][surface.dimensions - 1];
		...
		for (int i = 0; i < particles.Length; i++) {
			...
			NoiseSample sample = NoiseHash.Sum(method, point, surface.frequency, surface.octaves, surface.lacunarity, surface.persistence);
			sample = sample * 0.5f;
			sample *= amplitude;
