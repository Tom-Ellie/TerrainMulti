...
public class Flow3D : MonoBehaviour {
	...
	private void PositionParticles () {
		...
		NoiseMethod method = NoiseHash.methods[(int)type][dimensions - 1];
		...
		for (int i = 0; i < particles.Length; i++) {
			...
			NoiseSample sampleX = NoiseHash.Sum(method, point, frequency, octaves, lacunarity, persistence);
			sampleX *= amplitude;
			sampleX.derivative = qInv * sampleX.derivative;
			point = q * new Vector3(position.x + 100f, position.z, position.y + morphOffset) + offset;
			NoiseSample sampleY = NoiseHash.Sum(method, point, frequency, octaves, lacunarity, persistence);
			sampleY *= amplitude;
			sampleY.derivative = qInv * sampleY.derivative;
			point = q * new Vector3(position.y, position.x + 100f, position.z + morphOffset) + offset;
			NoiseSample sampleZ = NoiseHash.Sum(method, point, frequency, octaves, lacunarity, persistence);
			sampleZ *= amplitude;
