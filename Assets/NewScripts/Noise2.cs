...
public static class Noise2 {
    ...
    public static float[,] GenerateNoiseMap(int mapSize, NoiseSettings settings, Vector2 sampleCentre) {
        ...
        //Go  contain offset on each octave
        Vector2[] octaveOffsets = new Vector2[settings.octaves];

        //Max possible height for clamping
 ...
 //                   float sampleY = (y - halfHeight + octaveOffsets[i].y) / settings.scale * frequency;
                    ...
                    if (settings.type == NoiseMethodType.Perlin) {
                        ...
                        sampleX = (x + octaveOffsets[i].x) / settings.scale;
                        sampleY = (y + octaveOffsets[i].y) / settings.scale;
...
public class NoiseSettings {
    public Noise2.NormalizeMode normalizeMode;
